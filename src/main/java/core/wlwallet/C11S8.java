...
public class C11S8 {
	...
	public static void main(String[] args) {
		...
		tree.nodes.set(5, hashes);
		
		while(tree.root()[0]==0x00){
			if(tree.isLeaf()){
				tree.up();
			}else{
				byte[] leftHash = tree.getLeftNode();
				if(leftHash[0]==0x00){
					tree.left();
				}else if(tree.rightExists()){
					byte[] rightHash = tree.getRightNode();
					if(rightHash[0]==0x00){
						tree.right();
					}else{
					...
					tree.setCurrentNode(Utils.merkleParent(leftHash, leftHash));
					tree.up();
				}
			}
		}
